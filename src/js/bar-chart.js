function zoomBarChart() {
    ...
    // Add lines to create a border for the chart.
    // Add a line bisecting the chart at 0, providing
    // another visual que differentiating positive
    // from negative numbers.
    function renderGuideLines() {

        // Bisect the chart at zero
        _chart.append('line')
                .attr('x1', 0)
                .attr('y1', _yChart(0))
                .attr('x2', drawWidth)
    ...
    // Renders a static legend in the left hand corner of
    // the chart
    function renderLegend() {
        var padding = 20;
        var legend = svg.append('g')
                .attr('class', 'legend')
                .attr('transform', 'translate(' + (chartMargin.left + padding) + 
                    ',' + (chartMargin.top + padding) + ')');
